...
CLI のオプション（`hma serve` は argv を読まない。環境変数か設定ファイルで渡す）:

| | |
|---|---|
| `--thread <id>` | スレッドを指定（対話は既定 `cli`、`-p` は既定で毎回新しい） |
| `--new` | 新しいスレッドを立てる（`--thread` より優先） |
| `-p <prompt>` | 非対話。1回だけ答えて終わる（`-p -` で stdin から読む） |
| `--profile <name>` | `sandbox` / `coding` |
| `--workspace <path>` | 触ってよいディレクトリ |

**`-p` は入力を待てない。** 承認が要るツールに当たるとその場で止まるので、`APPROVAL=auto` か
allow ルールと併せて使う。答えは stdout、進捗と警告は stderr に出る。
終了コードは `0` 完走 / `1` エラー / `2` 承認が要って止まった。
