...
ステップ18 を入れたあと、差分をレビューして3つ見つかった。どれも「preflight は逐次、
...
対策は `ToolCallContext` に `waitForRunning` を足し、**書き込む側のフックだけが先行を待つ**。
...
`runTool` 全体を try/catch で包んで**投げないこと**。ツール本体の例外は
`agent/tools.ts` がすでに文字列にしているが、`afterToolCall` は投げうる。
