...
function ToolApproval() {
  useInterrupt({
    enabled: ({ value }) => (value as { reason?: string })?.reason === "tool_approval",
    render: ({ interrupt, resolve }) => {
      const meta = interrupt?.metadata as { name?: string; arguments?: string } | undefined;
      return (
        <div
          style={{
        ...
        >
          ...
          {meta?.arguments && (
            <pre style={{ background: "#f5f4f0", padding: 8, overflowX: "auto" }}>
